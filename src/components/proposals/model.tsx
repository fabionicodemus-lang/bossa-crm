...
export type ProposalForm = { origin:Origin; leadId:string; clientName:string; developmentId:string; unitId:string; workflowStatus:WorkflowStatus; proposalDate:string; validUntil:string; listPrice:string; entryTotal:string; hasBeforeKeysMonthly:boolean; beforeKeysCount:string; beforeKeysAmount:string; reinforcementFrequency:ReinforcementFrequency; reinforcementCount:string; reinforcementAmount:string; firstReinforcementDate:string; keysAmount:string; hasAfterKeysMonthly:boolean; afterKeysCount:string; afterKeysAmount:string; nextAction:string; nextActionDueAt:string; notes:string };
export type ProposalCalculations = { listPrice:number; proposedPrice:number; entryTotal:number; paidUntilKeys:number; paidUntilKeysPercent:number; nominalTotal:number; discountAmount:number; discountPercent:number; differenceFromTable:number; scheduleItems:ScheduleItem[]; paidBeforeMonthlyCount:number; paidReinforcementCount:number };
...
export function planBoolean(plan:Record<string,unknown>|null|undefined,key:string,fallback:boolean){ const value=plan?.[key]; return typeof value==='boolean'?value:fallback }
...
export function blankForm(initialLead?:ProposalLead):ProposalForm{ return { origin:initialLead?.kind??'cliente', leadId:initialLead?.id??'', clientName:initialLead?.kind==='cliente'?initialLead.name:'', developmentId:'', unitId:'', workflowStatus:'rascunho', proposalDate:localToday(), validUntil:'', listPrice:'', entryTotal:'', hasBeforeKeysMonthly:false, beforeKeysCount:'', beforeKeysAmount:'', reinforcementFrequency:'anual', reinforcementCount:'', reinforcementAmount:'', firstReinforcementDate:'', keysAmount:'', hasAfterKeysMonthly:false, afterKeysCount:'', afterKeysAmount:'', nextAction:'', nextActionDueAt:'', notes:'' } }
