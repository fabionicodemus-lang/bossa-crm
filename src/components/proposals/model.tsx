...
export type ScheduleItem = {
  ...
  total: number;
  paidUntilKeysQuantity: number;
  paidUntilKeysAmount: number;
};
...
export function planText(plan: Record<string, unknown> | null | undefined, key: string) {
...
export function addMonthsIso(value: string, months: number) {
  ...
  source.setDate(Math.min(originalDay, new Date(source.getFullYear(), source.getMonth() + 1, 0).getDate()));
  return isoFromDate(source);
}

export function isOnOrBefore(value: string, limit: string | null | undefined) {
  return Boolean(value && limit && value <= limit);
}

export function countOccurrencesUntil(startDate: string, intervalMonths: number, quantity: number, limitDate: string | null) {
  if (!startDate || !limitDate || quantity <= 0) return 0;
  let count = 0;
  for (let index = 0; index < quantity; index++) {
    if (isOnOrBefore(addMonthsIso(startDate, index * intervalMonths), limitDate)) count++;
  }
  return count;
}

export function scheduleItemsTotal(items: ScheduleItem[]) {
  return items.reduce((sum, item) => sum + numberValue(item.total), 0);
}

export function proposalScheduleWarnings(
  form: ProposalForm,
  ...
  if (hasKeysPayment && !deliveryDate) {
    warnings.push('A proposta possui parcela nas chaves, mas falta a Data da Entrega. Cadastre-a em Comercial → Empreendimentos, editando o empreendimento selecionado.');
  }
  if ((hasUnifiedMonthly || hasBeforeKeysMonthly) && !deliveryDate) {
    warnings.push('A proposta possui parcelas vinculadas às chaves, mas falta a Data da Entrega para separar corretamente os vencimentos. Cadastre-a em Comercial → Empreendimentos.');
  }
  if (monthlyAfterCount > 0 && !firstAfterKeysDate) {
    warnings.push('A proposta possui parcelas pós-chaves, mas falta a data da primeira parcela pós-chaves. Cadastre a Data da Entrega em Comercial → Empreendimentos; o CRM calcula o primeiro vencimento para um mês depois.');
  }
...
export function formatMoneyInput(value: string) {
  return value === '' ? '' : money.format(numberValue(value));
}
...
export function workflowOf(proposal: Proposal): WorkflowStatus {
...
export function storedStatusOf(status: WorkflowStatus): StoredStatus {
  if (status === 'negociacao' || status === 'contraproposta') return 'enviada';
  if (status === 'convertida') return 'aprovada';
  return status;
}

export function suggestedReinforcementDate(proposalDate: string, frequency: ReinforcementFrequency) {
  return addMonthsIso(proposalDate, frequency === 'semestral' ? 6 : 12);
}
...
export function MoneyInput({
  ...
  return <input
    ...
    disabled={disabled}
    readOnly={readOnly}
    style={readOnly ? { background: 'var(--bg)', fontWeight: 800 } : undefined}
  />;
}
...
export function calculateProposal(
  ...
  if (form.monthlyMode === 'unificado') {
    ...
    if (monthlyBeforeCount > 0) {
      scheduleItems.push({
        kind: 'parcela_ate_chaves',
        label: 'Parcelas mensais até as chaves',
        quantity: monthlyBeforeCount,
        amount: monthlyAmount,
        startDate: firstMonthlyDate || null,
        intervalMonths: 1,
        total: beforeMonthlyTotal,
        paidUntilKeysQuantity: monthlyBeforeCount,
        paidUntilKeysAmount: beforeMonthlyTotal,
      });
    }
    if (monthlyAfterCount > 0) {
      const firstAfterDate = deliveryDate
        ? addMonthsIso(firstMonthlyDate, monthlyBeforeCount)
        : null;
      scheduleItems.push({
        kind: 'parcela_pos_chaves',
        label: 'Parcelas mensais após as chaves',
        quantity: monthlyAfterCount,
        amount: monthlyAmount,
        startDate: firstAfterDate,
        intervalMonths: 1,
        total: afterMonthlyTotal,
        paidUntilKeysQuantity: 0,
        paidUntilKeysAmount: 0,
      });
    }
  } else {
    ...
    if (monthlyBeforeCount > 0) {
      scheduleItems.push({
        ...
        intervalMonths: 1,
        total: beforeMonthlyTotal,
        paidUntilKeysQuantity: paidBeforeCount,
        paidUntilKeysAmount: paidBeforeCount * beforeAmount,
      });
    }
    if (monthlyAfterCount > 0) {
      scheduleItems.push({
        ...
        quantity: monthlyAfterCount,
        amount: afterAmount,
        startDate: firstAfterKeysDate || null,
        intervalMonths: 1,
        total: afterMonthlyTotal,
        paidUntilKeysQuantity: 0,
        paidUntilKeysAmount: 0,
  ...
  const paidReinforcementCount = countOccurrencesUntil(
    form.firstReinforcementDate,
  ...
  const paidUntilKeys = entryTotal
    + paidMonthlyAmount
    + paidReinforcementCount * reinforcementAmount
    + (deliveryDate ? keysAmount : 0);

  if (entryTotal > 0) {
    const entryPaidUntilKeys = Boolean(deliveryDate && isOnOrBefore(form.proposalDate, deliveryDate));
    scheduleItems.unshift({
      kind: 'entrada',
      label: 'Entrada direta',
      quantity: 1,
      amount: entryTotal,
      startDate: form.proposalDate || null,
      intervalMonths: null,
      total: entryTotal,
      paidUntilKeysQuantity: entryPaidUntilKeys ? 1 : 0,
      paidUntilKeysAmount: entryPaidUntilKeys ? entryTotal : 0,
    });
  }
  if (reinforcementCount > 0) {
    scheduleItems.push({
      kind: form.reinforcementFrequency === 'semestral' ? 'reforco_semestral' : 'reforco_anual',
      label: form.reinforcementFrequency === 'semestral' ? 'Reforços semestrais' : 'Reforços anuais',
      quantity: reinforcementCount,
      amount: reinforcementAmount,
      startDate: form.firstReinforcementDate || null,
      intervalMonths: reinforcementInterval,
      total: reinforcementTotal,
      paidUntilKeysQuantity: paidReinforcementCount,
      paidUntilKeysAmount: paidReinforcementCount * reinforcementAmount,
    });
  }
  if (keysAmount > 0) {
    scheduleItems.push({
      ...
      intervalMonths: null,
      total: keysAmount,
      paidUntilKeysQuantity: deliveryDate ? 1 : 0,
      paidUntilKeysAmount: deliveryDate ? keysAmount : 0,
    });
  }

  const scheduleOrder: Record<string, number> = {
    entrada: 0,
    parcela_ate_chaves: 1,
    reforco_semestral: 2,
    reforco_anual: 2,
    chaves: 3,
    parcela_pos_chaves: 4,
  };
  scheduleItems.sort((first, second) => (scheduleOrder[first.kind] ?? 9) - (scheduleOrder[second.kind] ?? 9));

  // O total oficial nasce das mesmas linhas que serão salvas e exibidas no PDF.
  // Assim, nenhuma parcela pode existir no total sem existir também no cronograma.
