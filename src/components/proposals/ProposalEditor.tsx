...
import { date, dateFromIso, integerValue, money, MoneyInput, ToggleButton, type Origin, type ProposalCalculations, type ProposalDevelopment, type ProposalForm, type ProposalLead, type ProposalUnit, statusLabels, type WorkflowStatus } from './model';

export function ProposalEditor({form,setField,chooseOrigin,chooseLead,chooseDevelopment,chooseUnit,developments,availableLeads,developmentUnits,selectedLead,selectedDevelopment,selectedUnit,deliveryDate,firstBeforeKeysDate,firstAfterKeysDate,maxBeforeKeysCount,calculations,canEdit,saving,editingId,onCancel,onSubmit}:{
  form:ProposalForm;
  ...
  return <form onSubmit={onSubmit}>
    <div className="page-head"><div><h2>{editingId?'Editar proposta':'Nova proposta'}</h2><p>O valor da proposta e o percentual até as chaves são calculados automaticamente pelas datas.</p></div><button type="button" className="btn btn-ghost" onClick={onCancel}>← Voltar para propostas</button></div>
      ...
      <div style={{display:'grid',gap:14}}>
        <section className="card"><div className="card-head"><h3>1. Origem e produto</h3></div><div className="card-body">
          <div className="grid grid-2">
            ...
            <div className="field"><label>Data da proposta</label><input className="input" type="date" value={form.proposalDate} onChange={event=>setField('proposalDate',event.target.value)} disabled={!canEdit}/></div>
        ...
        <section className="card"><div className="card-head"><h3>2. Valores e fluxo de pagamento</h3></div><div className="card-body">
          ...
          <div className="card" style={{boxShadow:'none',marginTop:14}}><div className="card-head"><h3>Entrada e chaves</h3><span className="chip">Pagamentos diretos</span></div><div className="card-body grid grid-2">
          ...
          <div className="card" style={{boxShadow:'none',marginTop:14}}><div className="card-head"><h3>Parcelas mensais</h3><div className="page-actions"><ToggleButton active={form.hasBeforeKeysMonthly} onClick={()=>setField('hasBeforeKeysMonthly',!form.hasBeforeKeysMonthly)} disabled={!canEdit}>Antes das chaves</ToggleButton><ToggleButton active={form.hasAfterKeysMonthly} onClick={()=>setField('hasAfterKeysMonthly',!form.hasAfterKeysMonthly)} disabled={!canEdit}>Depois das chaves</ToggleButton></div></div><div className="card-body">
          ...
          <div className="card" style={{boxShadow:'none',marginTop:14}}><div className="card-head"><h3>Reforços</h3><div className="page-actions"><ToggleButton active={form.reinforcementFrequency==='anual'} onClick={()=>setField('reinforcementFrequency','anual')} disabled={!canEdit}>Anuais</ToggleButton><ToggleButton active={form.reinforcementFrequency==='semestral'} onClick={()=>setField('reinforcementFrequency','semestral')} disabled={!canEdit}>Semestrais</ToggleButton></div></div><div className="card-body grid grid-3">
        ...
        <section className="card"><div className="card-head"><h3>3. Cronograma calculado</h3><span className="chip">Até a entrega entra no percentual</span></div><div className="table-wrap"><table><thead><tr><th>Pagamento</th><th>Qtd.</th><th>Valor</th><th>Primeiro vencimento</th><th>Total</th><th>Até chaves</th></tr></thead><tbody>
        ...
        <section className="card"><div className="card-head"><h3>4. Acompanhamento</h3></div><div className="card-body"><div className="grid grid-2">
