...
export function LeadLifecycleActions({
  ...
  return <>
    ...
    {dialog && <div role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(43,38,34,.48)', display: 'grid', placeItems: 'center', padding: 20 }} onMouseDown={(event) => { if (event.target === event.currentTarget) closeDialog(); }}>
      <section className="card" style={{ width: 'min(520px, 100%)', boxShadow: '0 18px 60px rgba(0,0,0,.24)' }}>
        <div className="card-head"><h3>{dialog === 'archive' ? 'Arquivar lead' : 'Excluir lead permanentemente'}</h3><button type="button" className="btn btn-ghost btn-sm" onClick={closeDialog}>Fechar</button></div>
        <div className="card-body">
          {dialog === 'archive' ? <>
            ...
            <div className="field"><label>Motivo do arquivamento</label><textarea className="textarea" value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Ex.: lead de teste, cadastro duplicado ou contato sem interesse." /></div>
            ...
            <div className="field"><label>Digite exatamente o nome para confirmar</label><div className="faint" style={{ marginBottom: 7 }}>{leadName}</div><input className="input" value={confirmName} onChange={(event) => setConfirmName(event.target.value)} autoComplete="off" /></div>
            {error && <div className="error-box">{error}</div>}
