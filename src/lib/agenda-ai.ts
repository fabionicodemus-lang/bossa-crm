...
function parseDateFromText(raw: string, now = new Date()): string | null {
  ...
  for (const [name, target] of Object.entries(weekdays)) {
    if (!text.includes(normalize(name))) continue;
...
function parseTimeFromText(raw: string): string | null {
  const text = normalize(raw);
  const explicit = text.match(/(?:\bas\s+|\bàs\s+)?\b([01]?\d|2[0-3])[:h]([0-5]\d)?\b/);
...
function appointmentMode(text: string) {
  ...
  if (/\b(video|videochamada|meet|online)\b/.test(value)) return 'video' as const;
...
export async function maybeScheduleAgendaFromAi(args: {
  admin: AdminClient;
  organizationId: string;
  lead: Lead;
  turn: AiTurn;
  lastUserMessage: string;
}): Promise<AgendaAiResult> {
  ...
  const currentText = `${args.lastUserMessage}\n${args.turn.reply}\n${args.turn.next_action}`;
  const date = parseDateFromText(currentText) || parseDateFromText(recentText);
  const time = parseTimeFromText(currentText) || parseTimeFromText(recentText);

  if (!date || !time) {
    return { status: 'needs_details', message: 'Para marcar, preciso confirmar a data e o horário exatos.' };
  }
  ...
  const { data: duplicate } = await args.admin.from('agenda_events')
    .select('id,assigned_to,starts_at,ends_at')
    ...
    .maybeSingle();
  if (duplicate) {
    return { status: 'created', eventId: duplicate.id, assignedTo: duplicate.assigned_to, startsAt: duplicate.starts_at, endsAt: duplicate.ends_at };
  }
  ...
  for (const userId of candidates) {
    ...
    if (args.lead.owner_id) break;
  }

  if (!assignedTo) {
    return {
      status: 'conflict',
      message: preferredBusy
  ...
  const { data: event, error } = await args.admin.from('agenda_events').insert({
    ...
    created_by_kind: 'ai',
    agent: args.lead.kind === 'cliente' ? 'nara' : 'plantao',
    title: `${label} · ${args.lead.name}`,
    description: args.turn.summary || args.turn.next_action || null,
    event_type: type,
    meeting_mode: mode,
    starts_at: startsIso,
    ends_at: endsIso,
    metadata: {
      ...
      last_user_message: args.lastUserMessage,
