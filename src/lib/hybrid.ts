...
const AI_STAGES = new Set<WorkflowStage>(['novo_triagem', 'qualificacao_ia', 'nutricao_ativa', 'passagem_pendente', 'futuro']);
const HUMAN_STAGES = new Set<WorkflowStage>(['humano_ativo', 'agendado', 'pos_reuniao', 'proposta_negociacao']);

function normalize(value: string): string {
  return value.normalize('NFD').replace(/\p{Diacritic}/gu, '').toLocaleLowerCase('pt-BR').replace(/\s+/g, ' ').trim();
}
function minutesFrom(now: Date, minutes: number): string { return new Date(now.getTime() + minutes * 60_000).toISOString(); }
function daysFrom(now: Date, days: number): string { return new Date(now.getTime() + days * 86_400_000).toISOString(); }

function leadStage(lead: Lead): WorkflowStage {
  const stage = lead.stage as WorkflowStage;
  return AI_STAGES.has(stage) || HUMAN_STAGES.has(stage) || stage === 'fechado_ganho' || stage === 'encerrado'
    ? stage
    : 'novo_triagem';
}
function currentOwnerMode(lead: Lead): OwnerMode {
  return lead.owner_mode === 'human' || lead.owner_mode === 'none' ? lead.owner_mode : 'ai';
}
...
export function aiCanReply(lead: Lead): boolean {
  return Boolean(lead.ai_enabled && !lead.opt_out && !lead.automation_paused && currentOwnerMode(lead) === 'ai' && AI_STAGES.has(leadStage(lead)));
}

export function deriveHybridDecision({ lead, turn, lastUserMessage, now = new Date() }: {
  lead: Lead; turn: AiTurn; lastUserMessage: string; now?: Date;
}): HybridDecision {
  ...
  // Fechamento automático só é aceito durante uma negociação humana já aberta.
  ...
  if (won) {
    stage = 'fechado_ganho'; resolvedOwnerMode = 'none'; aiEnabled = false;
    nextAction = 'Registrar fechamento e iniciar o processo de pós-venda.'; nextActionType = 'pos_venda';
  } else if (isOptOut || priorityClass === 'D') {
    stage = 'encerrado'; resolvedOwnerMode = 'none'; aiEnabled = false;
    nextAction = isOptOut ? 'Respeitar o opt-out definitivo.' : nextAction; nextActionType = 'encerrar';
  } else if (alreadyHuman) {
    resolvedOwnerMode = 'human'; aiEnabled = false;
    if (turn.stage === 'agendado' || currentStage === 'agendado' || appointmentSignal(lastUserMessage)) stage = 'agendado';
    ...
    taskDescription = nextAction; taskPriority = priorityClass === 'A1' ? 'urgent' : priorityClass === 'A2' ? 'high' : 'normal';
    taskDueAt = nextActionDueAt; taskDedupeKey = `human:${stage}:${nextActionType}`;
  } else if (handoffRequired) {
    stage = 'passagem_pendente'; resolvedOwnerMode = 'ai'; aiEnabled = true;
    nextActionDueAt = minutesFrom(now, priorityClass === 'A1' ? 5 : 30); nextActionType = 'aceitar_passagem';
    nextAction = priorityClass === 'A1' ? 'Um consultor deve aceitar a passagem e iniciar o contato imediatamente.' : 'Um consultor deve aceitar a passagem e assumir o atendimento no mesmo turno.';
    taskTitle = priorityClass === 'A1' ? 'Aceitar lead A1' : 'Aceitar passagem do lead';
    taskDescription = highIntentReason || turn.summary || turn.next_action; taskPriority = priorityClass === 'A1' ? 'urgent' : 'high';
    taskDueAt = nextActionDueAt; taskDedupeKey = 'handoff:pending';
  } else if (future || currentStage === 'futuro') {
    stage = 'futuro'; resolvedOwnerMode = 'ai'; aiEnabled = true;
    reactivationAt = lead.reactivation_at || daysFrom(now, 30); nextActionDueAt = reactivationAt; nextActionType = 'reativar';
    nextAction = turn.next_action || 'Retomar o contato na data de reativação, usando o motivo real da pausa.';
    taskTitle = 'Reativar lead futuro'; taskDescription = nextAction; taskPriority = 'low'; taskDueAt = reactivationAt; taskDedupeKey = 'reactivation:future';
  } else {
    stage = currentStage === 'novo_triagem' ? 'qualificacao_ia' : 'nutricao_ativa'; resolvedOwnerMode = 'ai'; aiEnabled = true;
    nextActionDueAt = priorityClass === 'A2' ? minutesFrom(now, 30) : priorityClass === 'C' ? daysFrom(now, 7) : minutesFrom(now, 24 * 60);
    nextActionType = 'qualificar_nutrir'; taskTitle = 'Próxima ação da Nara/Plantão'; taskDescription = nextAction;
    taskPriority = priorityClass === 'A2' ? 'high' : 'normal'; taskDueAt = nextActionDueAt; taskDedupeKey = `ai:${stage}`;
  }
  ...
  return {
    stage, priorityClass, ownerMode: resolvedOwnerMode, aiEnabled, handoffRequired, handoffReason,
    nextAction, nextActionType, nextActionDueAt, reactivationAt,
    noteTitle: `${label} analisou a conversa · classe ${priorityClass}`,
    noteDescription: [turn.summary, turn.next_action ? `Próxima ação sugerida: ${turn.next_action}` : '', handoffReason ? `Motivo da passagem: ${handoffReason}` : ''].filter(Boolean).join(' '),
    taskTitle, taskDescription, taskPriority, taskDueAt, taskDedupeKey,
  };
}

export function workflowStageAllowsAi(stage: string): boolean { return AI_STAGES.has(stage as WorkflowStage); }
export function isHumanWorkflowStage(stage: string): boolean { return HUMAN_STAGES.has(stage as WorkflowStage); }
